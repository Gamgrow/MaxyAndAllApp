...
def doooo(text):
    ...
    if text in ["exit", "quit", "stop"]:
    ...
    elif ("close all tabs" in text) or ("close all application" in text):
        close_app("firefox")
        close_app("chrome")
        close_app("settings")
        close_app("brave")
        ...
        if yesnone !=1:
            ...
            elif text in ["opens the temporary files folder", "open the temporary files folder", "open temporary files folder", "temporary files folder open", "open temporary files", "temporary file open", "open temporary file", "temporary files open","temp","show temporary files","delete temporary files"]:
...
#  for icon
    ...
    # label 1
    mylabel = Label(wind,font=("arial",20),bg='#1570CB',fg='white',justify="left" ,border=1)
    ...
    # label 2
    mylabel2 = Label(wind,font=("arial",16 ),bg="#1570CB",justify="left"   )
    mylabel2.place(x=1000,y=85)




    my_pic = Image.open("searchicon.png")
    ...
    my_pic_wctm = Image.open("wctm.png")

    resize_pic_wctm = my_pic_wctm.resize((388, 124), Image.ANTIALIAS)
    my_pic_wctm = ImageTk.PhotoImage(resize_pic_wctm)
    mylabel2wctm = Label(wind,text = "Maxy\n Welcome",bg="#108cff",font=("Arial Black",18 ) , fg="white")
