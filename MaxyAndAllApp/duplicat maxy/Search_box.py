...
class searchbox():
    def doooo(text):
        ...
        if text in ["exit", "quit", "stop"]:
            ...
            print()
            if hour >= 12:
                main.printtext(hour, ":", minut, 'PM')
                speak(str(hour)+":"+str(minut)+'PM')
            else:
                main.printtext(hour, ":", minut, 'AM')
            ...
            name =Take_command.command()
            print(name)
            speak("tell me subject")
            subject = Take_command.command()
            print(subject)
            speak("what do you want to send")
            contant = Take_command.command()
            print(contant)

            for i in range(1, length):
                ans =Take_command.lower(li_name[i])
                if name == ans:
                    email = sh1.cell(i+1, 2).value

            Send_mail.sendmail(email, subject, contant, name)
            ...
            main.printtext("i am fine, and how are you")
            ...
            a = li[0]
            b = li[1]
            main.printtext("the answer is"+str(a/b))
            ...
            main.printtext("sorry i don't understand")
